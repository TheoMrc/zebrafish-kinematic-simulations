...
def main():
    """Main entry point for the zebrafish analysis application."""
    show_frames = True

    print(matplotlib.get_backend(), os.environ.get("COMPUTERNAME", ""))
    # Resolve paths relative to the repository root (location of this file)
    repo_root = Path(__file__).resolve().parent
    source_path = str(repo_root / "test" / "test_experiment")
    target_main_folder = str(repo_root / "test" / "tmp_results")
    ...
    for video_name in ["test_video"]:
        ...
        plt.show()
        np.savetxt(
            os.path.join(target_path, "kinematics_data", "mass_center_pos.dat"),
            video.mass_centers,
        )
        print(os.path.join("kinematics_data", "mass_center_pos.dat"), "saved!")
        ...
        np.savetxt(
            os.path.join(target_path, "kinematics_data", "Angle.dat"),
            np.array(video.smoothed_angles),
        )
        print(os.path.join("kinematics_data", "Angle.dat"), "saved!")
        ...
        Video.process_frames_from_smoothed_angle(
            ...
            save_frames=True,
            hdf5_path=os.path.join(target_path, "aligned.h5"),
            meta=None,  # optionally pass {'fps': ..., 'px_per_mm': ..., 'fish_length_mm': ...}
        )
